...
public class LeaveDAO {
	...
	public String approveDeny(int mgrId, int leaveId, String status, String managerComments)
		...
		if (leaveDetails != null) {
			...
			if (mgr == mgrId) {
				if (status.toUpperCase().equals("YES")) {
					...
					String cmd = "Update leave_history set LEAVE_STATUS=?, " + " LEAVE_MNGR_COMMENTS=? where "
							+ "Leave_ID=?";
	...
	public String applyLeave(LeaveDetails leaveDetails) throws ClassNotFoundException, SQLException {
		...
		if (days <= 0) {
			...
			if (leaveDetails.getLeaveType().name().equals(LeaveType.EL.toString())) {
				pst.setString(4, LeaveType.EL.toString());
			}
